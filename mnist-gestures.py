...
mlp_model = load_model('mnist_model.hdf5')
#cnn_model = load_model('emnist_cnn_model.h5')

# Letters lookup
letters = { 1: '1', 2: '2', 3: '2', 4: '4', 5: '5', 6: '6', 7: '7', 8: '8', 9: '9', 0: '0'}
...
while True:
    ...
    if len(cnts) > 0:
        ...
        cnt = max(cnts, key=cv2.contourArea) # sorted(cnts, key = cv2.contourArea, reverse = True)[0][0]
        ...
        if len(points) != 0:
            ...
            if blackboard_cnts is not None and len(blackboard_cnts) >= 1:
                ...
                if cv2.contourArea(cnt) > 1000:
                    ...
                    prediction1 = mlp_model.predict(newImage.reshape(1,784))[0]
                    prediction1 = np.argmax(prediction1)
                    print('Prediction is '+ str(prediction1))
    ...
    cv2.putText(frame, "Multilayer Perceptron : " + str(prediction1), (10, 410), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255, 255, 255), 2)
